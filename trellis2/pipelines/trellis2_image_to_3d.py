...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def preprocess_image(self, input: Image.Image, remove_background=None) -> Image.Image:
        """
        ...
        if (not has_alpha and (remove_background is None or remove_background)) or (remove_background is not None and remove_background):
    ...
    def decode_latent(
        self,
        shape_slat: SparseTensor,
        tex_slat: Optional[SparseTensor],
        resolution: int,
    ) -> Union[List[MeshWithVoxel], List[Mesh]]:
        """
        ...
        # If no texture latent provided, return the raw meshes only.
        if tex_slat is None:
            for m in meshes:
                m.fill_holes()
    ...
    def run(
        ...
        print(f"Running pipeline with type: {pipeline_type}")
        ...
        out_mesh = self.decode_latent(shape_slat, tex_slat if generate_texture else None, res)
        if return_latent:
            return out_mesh, (shape_slat, tex_slat if generate_texture else None, res)
        else:
            return out_mesh
