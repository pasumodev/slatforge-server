...
# This file was modified from the o_voxel codebase and incorporated into the SlatForge project.
...
def to_glb(
    ...
    faces: torch.Tensor,
    attr_volume: Optional[torch.Tensor] = None,
    ...
    If any texture baking arguments are None, skips baking and creates a simple material.
    
    Args:
        vertices: (N, 3) tensor of vertex positions
        faces: (M, 3) tensor of vertex indices
        attr_volume: (L, C) features of a sparse tensor for attribute interpolation (optional, None to skip baking)
        coords: (L, 3) tensor of coordinates for each voxel (optional, None to skip baking)
        attr_layout: dictionary of slice objects for each attribute (optional, None to skip baking)
        aabb: (2, 3) tensor of minimum and maximum coordinates of the volume
        voxel_size: (3,) tensor of size of each voxel (optional, None to skip baking)
        grid_size: (3,) tensor of number of voxels in each dimension (optional, None to skip baking)
        decimation_target: target number of vertices for mesh simplification
        texture_size: size of the texture for baking (optional, None to skip baking)
    ...
    if not remesh:
        # Remove aggressive simplification step to preserve more geometry, relying on the final cleanup to fix issues
    ...
    # --- UV Parameterization ---
    bake_args = [attr_volume, coords, attr_layout, voxel_size, grid_size, texture_size]
    skip_baking = any(arg is None for arg in bake_args)
    ...
    if not skip_baking:
        ...
        # Skip UV unwrapping, just use mesh as is
        if verbose:
            print("Skipping UV unwrapping due to missing baking arguments.")
        out_vertices, out_faces = mesh.read()
        out_vertices = out_vertices.cuda()
        out_faces = out_faces.cuda()
        mesh.compute_vertex_normals()
        out_normals = mesh.read_vertex_normals()
        # Dummy UVs: fill with zeros (or could use planar projection if needed)
        out_uvs = torch.zeros((out_vertices.shape[0], 2), device=out_vertices.device)
        out_vmaps = torch.arange(out_vertices.shape[0], device=out_vertices.device)
    ...
    if not skip_baking:
        ...
        if verbose:
            print("Skipping texture baking, creating simple material.")
        material = trimesh.visual.material.PBRMaterial(
